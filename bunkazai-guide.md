...
「磐田物語」の文化財コンテンツ群（`/c003.html/` 配下）を更新・追記する際の方針をまとめる。
...
| URL | 役割 |
|-----|------|
| `/c003.html/` (index.html) | 文化財トップ。コンテンツ群の入口 |
| `/c003.html/c013.html` | 国指定文化財一覧（8件カード） |
| `/c003.html/c012.html` | 古墳から見る磐田（地形・交通の考察） |
| `/c003.html/c015.html` | 埋蔵文化財と土地の記憶（不動産実務と接続） |
| `/c003.html/c005.html` | 文化財だより等への資料導線 |
| `/c003.html/c018.html` | 研究ノート（調査メモ蓄積） |

未作成（今後の拡張候補）：`/c003.html/c021.html`（全指定文化財一覧：県指定・市指定・国登録有形・無形民俗を含む）、遠江国分寺跡の文化財視点特集（既存 `/n016.html` と棲み分け）。
...
2. 該当一覧ページ（例 `c013.html`）に `.cpcard` を追加。バッジの色は種別で使い分け（`.badge` 史跡系、`.kinen` 記念物、`.minzoku` 民俗）。
...
- 聞き書き・現地調査メモ：`c018.html` のテンプレートに沿って追記。家・家族の記憶は本人/家族の許可と個人情報配慮を前提とする。
