...
6. **記事末尾の定型ブロック**：`<section class="local-property-note">`（不動産導線）と`<section class="article-policy">`（著者・参考資料・作成方針）を記事本文の後に配置。両方とも`iwata-area-color.css`にスタイル定義済みなので、クラス名を付けるだけでよい。
